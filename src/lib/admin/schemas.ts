 ...
 * Imagens NÃO entram — upload via FormData/File, fora do parse Zod de texto.
 */
export const updateProductSchema = z
  .object({
    ...
    lowStockThresholdGrams: z.number().int().min(0).optional(),
...
/**
 * Schema de input da criação de produto.
 * productType é obrigatório aqui (em editar fica travado).
 * incrementGrams NÃO entra — derivado no backend (100 granel / 1 unit).
 * Imagens NÃO entram — upload via FormData/File, fora do parse Zod de texto.
 */
export const createProductSchema = z
  .object({
    ...
    slug: z
      .string()
      .min(1, 'Identificador (slug) é obrigatório')
      .max(200, 'Slug deve ter no máximo 200 caracteres'),
    categoryId: z.number().int().positive('Categoria inválida'),
    productType: z.enum(['granel', 'unit'], { message: 'Tipo de produto inválido' }),
    description: z.string().max(2000, 'Descrição deve ter no máximo 2000 caracteres').optional().nullable(),
    priceCents: z.number().int().positive('Preço deve ser um valor positivo'),
    compareAtCents: z.number().int().positive('Preço original deve ser um valor positivo').optional().nullable(),
    stockQuantityGrams: z.number().int().min(0).optional().nullable(),
